    ...
    <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container mx-auto flex h-16 items-center justify-between px-4">
        <Link to="/" className="flex items-center gap-3">
          <img src="/assets/image.png" alt="Woods Academy Logo" className="h-10 w-10 object-contain" />
          <div className="flex flex-col">
            <span className="text-lg font-bold text-primary">WOODS ACADEMY</span>
            ...
            {navLinks.map((link) => (
              <Link
                ...
                className={`px-3 py-2 text-sm font-medium transition-colors hover:text-primary rounded-md ${
                  currentPath === link.path ? 'text-primary bg-accent' : 'text-muted-foreground'
                }`}
              >
                {link.label}
              </Link>
